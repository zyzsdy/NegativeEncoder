...
public class Muxer
{
        ...
        bool useHdr, string originInput, string extra)
    {
        ...
        var (ext, _) = FileName.GetOutputExt(preset.MuxFormat);
        var muxOutput = FileName.RecalcOutputPath(input, extra, "_mux", ext);
        if (input == originInput) muxOutput = extra;
        ...
        var extraArgs = "";
        if (preset.MuxFormat == OutputFormat.MP4) extraArgs = "-movflags faststart";
        var format = TaskArgBuilder.GetFormat(preset.MuxFormat);
        var args =
            $"-y -i \"{input}\" -i \"{audioInput}\" -map 0:v -map 1:a -c copy {extraArgs} -f {format} \"{muxOutput}\"";
