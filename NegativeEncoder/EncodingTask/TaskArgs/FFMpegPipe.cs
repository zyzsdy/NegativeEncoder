        ...
        bool useHdr, string originInput, string extra)
    {
        ...
        switch (param)
        {
            case "ProcessAudio":
            {
                ...
                batSb.Append(
                    $"\"{ffmpegFile}\" -y -i \"{input}\" -an -sn -pix_fmt yuv420p -f yuv4mpegpipe - | \"{encoderFile}\" --y4m {ioargs} {addArgs} {gargs}\n");
                ...
                batSb.Append(
                    $"\"{ffmpegFile}\" -y -i \"{input}\" -vn -sn -v 0 -c:a pcm_s16le -af aresample=async=1 -f wav pipe: | \"{qaacFile}\" -q 2 --ignorelength -c {preset.AudioBitrate} - -o \"{audioOutput}\"\n");
