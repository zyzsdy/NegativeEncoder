...
public static class TaskArgBuilder
{
    ...
    {
        if (preset.IsUseCustomParameters) return preset.CustomParameters;
        ...
        if (preset.IsSetMaxGop)
        {
            ...
            if (preset.IsStrictGop && preset.Encoder != Encoder.VCE) argList.Add("--strict-gop");
        }
        ...
        if (useHdr)
        {
            ...
            if (preset.IsRepeatHeaders && preset.Encoder == Encoder.NVENC) argList.Add("--repeat-headers");
            ...
            if (preset.IsConvertHdrType && preset.Encoder == Encoder.NVENC)
            {
                ...
                var param =
                    $"matrix={oldMatrix}:{newMatrix},colorprim={oldPrim}:{newPrim},transfer={oldTransfer}:{newTransfer}";
