...
namespace NegativeEncoder
{
    public class Options
    {
        ...
        [Option("runFunc", Required = false, HelpText = "执行一些功能，并退出。(SaveCmdTools|OpenCmdTools|InstallCmdTools|UninstallCmdTools)")]
    ...
    class Program
    {
            ...
            argOptionResult.WithParsed(o =>
                {
                    ...
                    if (!string.IsNullOrEmpty(o.BaseDir))
                    {
                        AppContext.EncodingContext.BaseDir = o.BaseDir;
                    }
        ...
        private static bool CheckBaseDir()
        {
            ...
            var fullPath = System.IO.Path.Combine(AppContext.EncodingContext.BaseDir, ffmpegFile);

            if (!System.IO.File.Exists(fullPath))
            {
                return MessageBox.Show("未检测到可用的消极压制工具包，请使用 --baseDir 参数指定消极压制工具包的目录。\n\n" +
                    "强制启动消极压制后核心功能无法使用，是否强制启动？", "初始化警告", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes;
            }
        ...
        private static void Init()
        {
            ...
            var binDir = System.IO.Path.GetDirectoryName(AppContext.EncodingContext.AppSelf);
            AppContext.EncodingContext.BaseDir = System.IO.Path.GetFullPath(System.IO.Path.Combine(binDir, ".."));
