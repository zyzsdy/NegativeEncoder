...
public class FileSelector
{
    ...
    public int BrowseImportFiles()
    {
        ...
        if (ofd.ShowDialog() == true) return AddFiles(ofd.FileNames);
        return -1;
    }
    ...
    public int AddFiles(string[] fileNames)
    {
        ...
        {
            var msg = "";
            if (errFiles.Count > 0) msg += $"以下 {errFiles.Count} 个文件已存在于列表中，未能导入：\n{string.Join("\n", errFiles)}\n\n";
            if (notFiles.Count > 0)
    ...
    public List<FileInfo> RemoveFiles(IList files)
    {
        var tempRemoveList = new List<FileInfo>();

        if (files != null && files.Count > 0)
        {
            foreach (var f in files) tempRemoveList.Add(f as FileInfo);
